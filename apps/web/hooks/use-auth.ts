...
// ─── useAuth() — T1.4 compatibility shim ─────────────────────────────────────
...
// Consumers should import from `@/hooks/use-auth` (this file) for the
// drop-in Zustand-like surface. The T1.3 hook can be consumed directly from
// `@/hooks/useAuth` by components that explicitly want the production flow.
//
// The Zustand store is marked `@deprecated` in `stores/auth-store.ts` and
// kept alive for:
//   - Sprint 1 demo mode.
 ...
 * Noop in demo mode.
 */
export async function prefetchCurrentUser(qc: ReturnType<typeof useQueryClient>): Promise<void> {
