...
// Moved from `apps/web/stores/auth-store.ts` as part of the feature-based
// refactor. The old location re-exports from here, so the 20+ pages that
// still import `useAuthStore` keep working unchanged.
//
// @deprecated Component-level consumers should migrate to `useAuth()` from
// `@/features/auth` — it encapsulates the demo-mode feature flag + TanStack
// Query cache wiring. This store is kept for:
//   - demo-mode authentication (offline / no backend)
//   - the NestJS auth API migration bridge (Sprint 1, T1.4)
//   - static `useAuthStore.setState(...)` writes (register page)
// Scheduled for removal once Sprint 2 completes.
    ...
    (set, get) => ({
      ...
      login: async (email: string, password: string) => {
        ...
        try {
          const registered = JSON.parse(
            localStorage.getItem('strickin-registered-users') ?? '[]',
          );
        ...
        try {
          const { accessToken, refreshToken, user } = await api.login(
            email,
            password,
          );
      ...
      register: async (userData: {
        ...
        const cookieVal = JSON.stringify({
          state: { user, token, refreshToken: 'demo-refresh', isDemo: true },
        });
        document.cookie = `strickin-auth=${encodeURIComponent(
          cookieVal,
        )};path=/;max-age=${60 * 60 * 24 * 7};SameSite=Lax`;
